...
async function uppdateraKrypto() {
    try {
        const svar = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=sek&include_24hr_change=true');
        const data = await svar.json();
        
        const pris = data.bitcoin.sek;
        const trend = data.bitcoin.sek_24h_change;
        
        const prisEl = document.getElementById('krypto-pris');
        const trendEl = document.getElementById('krypto-trend');
        
        prisEl.innerText = pris.toLocaleString('sv-SE') + " kr";
        
        if (trend >= 0) {
            trendEl.innerText = `+${trend.toFixed(2)}% (Upp)`;
            trendEl.style.color = "var(--income)";
        } else {
            trendEl.innerText = `${trend.toFixed(2)}% (Ner)`;
            trendEl.style.color = "var(--expense)";
        }
    } catch (fel) {
        console.log("Kunde inte hämta kryptopris:", fel);
        document.getElementById('krypto-pris').innerText = "Ej tillgängligt";
    }
}

async function uppdateraHelgdagar() {
    try {
        const idag = new Date();
        const ar = idag.getFullYear();
        const manad = idag.getMonth() + 1;
        
        const svar = await fetch(`https://sholiday.faboul.se/dagar/v2.1/${ar}/${manad}`);
        const data = await svar.json();
       
        const dagensDag = idag.getDate();
        const nastaHelgdag = data.dagar.find(d => Number(d.dag) >= dagensDag && d.helgdag);
        
        if (nastaHelgdag) {
            document.getElementById('helgdag-namn').innerText = nastaHelgdag.helgdag;
            document.getElementById('helgdag-datum').innerText = nastaHelgdag.datum;
        } else {
            document.getElementById('helgdag-namn').innerText = "Ingen fler denna månad";
        }
    } catch (fel) {
        console.log("Kunde inte hämta helgdagar:", fel);
        document.getElementById('helgdag-namn').innerText = "Ej tillgängligt";
    }
}
